...
- **State:** `/opt/wwtmc/data/state.json` (all site content)
- **Auth:** `/opt/wwtmc/data/auth.json` (password hash)
- **Uploads:** `/opt/wwtmc/public/uploads/` (gallery images)
...
Edit `/opt/wwtmc/server.js`, find the session cookie config, and change:
```javascript
...
```bash
sudo lsof -i :3001  # See what's using it
...
3. File permissions: `ls -la /opt/wwtmc/`
